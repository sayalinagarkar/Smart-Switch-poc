...
this.ws.addEventListener('close', (event) => {
  ...
  getMqttData() {
    let observable = new Observable(observer => {
      this._mqttService.observe('onpower/client5/from').subscribe((message: IMqttMessage) => 
      {
...
sendData(deviceInfo:any){
  ...
  {
    this._mqttService.unsafePublish("onpower/client5/to", JSON.stringify(deviceInfo), {qos: 0, retain: false});
  }
}
closeWebserver(){
